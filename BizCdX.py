...
st.set_page_config(page_icon="📇",page_title="BzCdx_OCR",layout = "wide")
st.title("OCR-Enhanced Business Card Data Collection and Organization")
#st.set_background_img(r"C:\Users\ADMIN\OneDrive\Pictures\bg.jpeg")
...
select=option_menu(
    ...
    options=["Home", "INSERTION AND CHANGE", "Delete"],
    icons=["at", "image","trash"],
    default_index=0,
...
if select == "Home":
  ...
  with col1:
    st.header(" :green[Description : ]") 
    st.write("Efficiently capture and digitize business card information using Optical Character Recognition (OCR) technology. Our Streamlit app simplifies the process of extracting and organizing contact details from business cards, ensuring accuracy and convenience.")
  with col2:
    st.image(Image.open(r"D:\Downloads1\1703248389437.jpeg"), width=500)

elif select == "INSERTION AND CHANGE":
  col1,col2=st.columns(2)
  with col1:
  ...
  if img is not None:
    ...
    text_image, IIMG= image_to_text(img)
    ...
    IIMG.save(Image_bytes, format= "PNG")
 ...
 # method =  st.radio("Select the Method",["None","Preview","Modify"])
  method = option_menu(
    menu_title="Select the Method",
    options=["None","Preview","Modify"],
    icons=["at","search", "image"],
    default_index=0,
    orientation="horizontal"
)
  if method == "None":
